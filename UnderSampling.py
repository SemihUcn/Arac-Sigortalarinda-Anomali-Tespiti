...
df_N_sampled = df_N.sample(n=247, random_state=42)


df_equal = pd.concat([df_Y, df_N_sampled], axis=0)


df_equal = df_equal.sample(frac=1, random_state=42).reset_index(drop=True)
# %%
df_equal.drop(columns=['_c39'],inplace=True)
# %%
y=df_equal['fraud_reported']
X = df_equal.drop(columns=["fraud_reported"])
# %%
