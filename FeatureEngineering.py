...
#%%
#------------------------------------------------------------------------------------------
X_new_feature = pd.DataFrame(index=X.index)
X_new_feature['customer_value_ratio'] = X['policy_annual_premium'].astype(float) / (X['months_as_customer'].astype(float) + 1)


X_new_feature['claim_ratio'] = (
    X['injury_claim'].astype(float) +
    X['property_claim'].astype(float) +
    X['vehicle_claim'].astype(float)
) / (X['total_claim_amount'].astype(float) + 1)

# 3. Müşteri olaydan önce kaç ay sistemdeydi
X_new_feature['customer_before_incident'] = (
    (X['incident_date_year'].astype(float) - X['policy_bind_date_year'].astype(float)) * 12 +
    (X['incident_date_month'].astype(float) - X['policy_bind_date_month'].astype(float))
)

# 4. Poliçe CSL toplamı ve oranı
X_new_feature['policy_csl_sum'] = X['policy_csl_min'].astype(float) + X['policy_csl_max'].astype(float)
X_new_feature['policy_csl_ratio'] = X['policy_csl_max'].astype(float) / (X['policy_csl_min'].astype(float) + 1)

# 5. Hafta sonu ve gece saatinde olay olmuş mu (20:00 sonrası)
X_new_feature['weekend_night_incident'] = (
    ((X['incident_date_is_weekend'].astype(int) == 1) & (X['incident_hour_of_the_day'].astype(float) > 20)).astype(int)
)

# 6. Erkek & hasar var & polis raporu var
X_new_feature['male_damage_reported'] = (
    (X['insured_sex_MALE'].astype(int) == 1) &
    (X['property_damage_YES'].astype(int) == 1) &
    (X['police_report_available_YES'].astype(int) == 1)
).astype(int)

# 7. Ortalama hasar miktarı
X_new_feature['mean_claim_value'] = (
    X['injury_claim'].astype(float) +
    X['property_claim'].astype(float) +
    X['vehicle_claim'].astype(float)
) / 3
X = pd.concat([X, X_new_feature], axis=1)
y = y.map({'Y': 1, 'N': 0})
...
# Korelasyon Matrisi (özellikler arasında)
...
print("Özniteliklerin hedef (fraud_reported) ile korelasyonu:\n")
print(feature_target_corr)

# Görselleştir
plt.figure(figsize=(10, 6))
sns.barplot(x=feature_target_corr.values, y=feature_target_corr.index, orient='h')
plt.title("Özniteliklerin Hedef ile Korelasyonu")
...
for col in numeric_cols:
    ...
    if len(data) > 5000:
        sample_data = data.sample(5000, random_state=42)  # shapiro limiti var
    else:
        sample_data = data
    ...
    # Yorumlama
    if shapiro_p > 0.05:
        distribution_type = 'Normal (Shapiro p > 0.05)'
    elif adf_p < 0.05:
        distribution_type = 'Durağan (ADF p < 0.05)'
    elif skewness > 1:
        distribution_type = 'Sağa Çarpık (Muhtemel Poisson)'
    elif skewness < -1:
        distribution_type = 'Sola Çarpık'
    else:
        distribution_type = 'Simetrik - Normal değil'

    distribution_summary.append({
        'Öznitelik': col,
        'Çarpıklık (Skewness)': skewness,
        'Basıklık (Kurtosis)': kurt,
        'Shapiro p': shapiro_p,
        'ADF p': adf_p,
        'Tahmini Dağılım': distribution_type
    })
...
import ace_tools as tools; tools.display_dataframe_to_user(name="Dağılım Özeti", dataframe=summary_df)

# %%
import seaborn as sns
import matplotlib.pyplot as plt

# Sayısal özniteliklerden normal dağılıma yakın olanları seç
normal_like_features = X.select_dtypes(include=['float64', 'int64']).columns[X.skew().abs() < 0.5]

# Yeterli sayıda varsa kovaryans matrisi hesapla ve görselleştir
if len(normal_like_features) >= 2:
    cov_matrix = X[normal_like_features].cov()

    # Kovaryans matrisini görselleştir
    plt.figure(figsize=(10, 8))
    sns.heatmap(cov_matrix, annot=True, fmt=".2f", cmap="viridis")
    plt.title("Normal Dağılıma Yakın Özellikler İçin Kovaryans Matrisi")
    plt.tight_layout()
    plt.show()
else:
    print("Yeterli sayıda normal dağılıma yakın öznitelik bulunamadı.")
...
for col in X.select_dtypes(include='int').columns:
    if X[col].nunique() < 15:  # örn: 15'ten az benzersiz değer
        categorical_cols.append(col)

# Yinelenenleri çıkar
categorical_cols = list(set(categorical_cols))

# İlk birkaç değişken için frekanslı çubuk grafikleri çiz
...
plt.title("UMAP ile 2D Veri Görselleştirme (Sınıf Etiketleriyle)")
plt.xlabel("UMAP 1")
plt.ylabel("UMAP 2")
